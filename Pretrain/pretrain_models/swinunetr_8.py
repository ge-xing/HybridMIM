...
class SwinUNETR(nn.Module):
    ...
    def __init__(
        ...
        for i in range(downsample_size+1):
            self.stages.append((select_reconstruct_region[0] * 2**i, select_reconstruct_region[1] * 2**i))
        print(self.stages)
        ...
        if pretrain:
            if feature_size == 24:
                self.pred_mask_region = nn.Linear(384, 65)# 一个region 8个 patch
                self.contrast_learning_head = nn.Linear(384, 384)
            else:
                self.pred_mask_region = nn.Linear(768, 65)# 一个region 8个 patch
                self.contrast_learning_head = nn.Linear(768, 384)
                self.pred_mask_region_position = nn.Linear(768, 64)
    ...
    def forward(self, x_in):
        ...
        images = x_in.detach()
        if self.pretrain:
            # mask_ratio = torch.clamp(torch.rand(1), 0.2, 0.4)
            mask_ratio = 0.4
            # x_in, mask = mask_func(x_in, self.in_channels, mask_ratio, (32, 32, 32), (4, 4, 4))
            x_in, mask = mask_func(x_in, self.in_channels, mask_ratio, (8, 8, 8), (12, 12, 12))

            region_mask_labels = get_mask_labels(x_in.shape[0], 3*3*3, mask, 4*4*4, device)
            region_mask_position = get_mask_labelsv2(x_in.shape[0], 3*3*3, mask, 4*4*4, device=device)
