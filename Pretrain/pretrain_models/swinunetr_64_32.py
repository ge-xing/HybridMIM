...
class SwinUNETR(nn.Module):
    ...
    def forward(self, x_in):
        ...
        images = x_in.detach()
        if self.pretrain:
            classifier_hidden_states = rearrange(hidden_states_out[-1], "b c (d m) (w n) (h l) -> b c d w h (m n l)", m=2, n=2, l=2)
            ...
            region_mask_labels = get_mask_labels(x_in.shape[0], 3*3*3, mask, 2*2*2, device)
            region_mask_position = get_mask_labelsv2(x_in.shape[0], 3*3*3, mask, 2*2*2, device=device)
